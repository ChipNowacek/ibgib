defmodule WebGib.Data.Schemas.TokenModel do
  ...
  schema "email_login_tokens" do
    ...
    field :token, :string
    field :ident_pin, :string

    timestamps([usec: true])
  end

  @required_fields ~w(email_addr token ident_pin)
  @optional_fields ~w()

  def changeset(content, params \\ :empty) do
    content
    |> cast(params, @required_fields, @optional_fields)
    |> validate_required([:email_addr, :token, :ident_pin])
    |> validate_length(:email_addr,
                       min: @min_email_addr_size,
                       max: @max_email_addr_size)
    |> validate_length(:token, min:
                       @hash_length,
                       max: @hash_length)
    |> validate_length(:ident_pin,
                       min: @min_ident_pin_size,
                       max: @max_ident_pin_size)
